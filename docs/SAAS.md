...
| GAP-028     | Health page copy contradicts the probe                     | ⚠️ Opened 2026-08-30 by GAP-011. `AdminPlatformHealthPage` tells the reader "samples are only written when an administrator opens this page" and "a scheduled probe is not running yet". Both become false the moment `0076` applies. The copy lives in the branch that closed BUG-059, so whichever of the two merges second must correct it. The page can also now show `measured_from` / `latency_from` and stop guessing which source it is displaying.                                                                                                         | P2       |
