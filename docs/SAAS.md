...
- [x] CAP-046 🟡 Rate limiting — org creation, invitations and the AI all capped, and the AI call has a
      token ceiling; the auth endpoints are GoTrue's own and unverified (GAP-031)
      `supabase/migrations/0085_rate_limiting.sql` · GAP-009 closed #199 · 10 pgTAP assertions
...
- [ ] CAP-076 🟡 Feature flags — `my_feature_access` returns them and the AI gate honours a flag as well
      as a plan, so they gate one thing; `beta_integrations` still gates nothing
      `supabase/migrations/0022_feature_flags.sql` · BUG-064 · P2
...
| BUG-063     | Own-activity screen is empty | Shows what the user did                     | Cited by `0071` and by CAP-057a since this session and never given a row until 2026-08-30 — the same lapse that left `0070` pointing at a BUG-062 nobody had written down. 291 of production's 331 org-visible audit rows carry no `actor_user_id`, because they were written by triggers firing with no `auth.uid()` (migrations, service_role, seed). Widening the policy would surface none of them; the fix is for the trigger bodies to record who acted. — `supabase/migrations/0016_audit_events.sql`                                                                                                                                                                                                                                                                                                                                                                                                                                                         | P2       |
...
| GAP-032     | Colour-contrast debt in DARK mode                          | ⚠️ Opened 2026-08-30 by the change that closed GAP-030, and **bigger than the light-mode debt was**: 195 nodes locally and 200 in CI, across the same 26 screens. Nothing had ever measured it — CI scanned light mode only, so this had been shipping unseen the whole time. It is a different problem with a different fix: `#3B6FE0` primary and `#D94A3A` danger are too **dark** against a `#111A2E` surface, where the light-mode failure was the same colours being too light against white. Closing it means a set of dark-side inks, which is a palette decision rather than a class swap, so it is tracked rather than bundled into the change that found it. Capped by `DARK_CONTRAST_BUDGET` in `e2e/app-surface.spec.ts` so it can only go down.                                                                                                                                                                                                                                                                                                                                                                                             | P2       |
| GAP-031     | Auth endpoints have no rate limit we control               | ⚠️ Opened 2026-08-30 by GAP-009, which covered the three surfaces this repository owns. Login, signup and password reset are GoTrue's, and their limits live in the Supabase dashboard under Auth → Rate Limits — not in a migration, and not verifiable from here. Somebody has to open that screen, read what the limits actually are, and write them down. Until then the answer to "what stops credential stuffing" is "whatever the defaults happen to be".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | P2       |
...
| HARDEN-010     | AI function                | One model, no fallback. `OPENROUTER_MODEL` (default `openai/gpt-4o-mini`) has no second choice, so an outage or a model deprecation is a 502 to the manager with nothing behind it. Split out of HARDEN-004 on 2026-08-30, which was closed on its cost half: a fallback is a resilience decision with its own cost implications, and folding it into a spend-control change would have been the wrong argument in the wrong PR.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | P2       |
...
indexes and the builder N+1 · a real authenticated E2E session (GAP-010) · the dark-mode contrast debt (GAP-032; light mode is closed) · the DPA once counsel drafts it · label the audit actions.
