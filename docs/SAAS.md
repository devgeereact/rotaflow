...
**Four of these are blocked by a decision, not by outstanding work**, and the difference
matters when reading the column: GAP-016 (stage 4), GAP-018 (stage 5), GAP-019 and GAP-020
(stage 6) and GAP-023 (stage 7) are all ⚫ in §7 — Phase 2 or deferred on evidence, each with
the condition that reopens it. Stage 5 also carries GAP-017, which is genuinely half open (the
tenant half of MFA). So the ladder above stage 3 is not a queue anybody is working through; it
is what the product would have to decide to become. Stages 1-3 are the ones with work in them,
and all three now turn on things this repository cannot do to itself: a restore, a real charge,
and a phone.
