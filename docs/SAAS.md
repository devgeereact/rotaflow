...
| GAP-030     | Colour-contrast debt across the authenticated app          | ⚠️ Opened 2026-08-30, and **larger than previously recorded**. The app-wide audit left "21 in Badge.tsx"; the real figure, now measured by CI, is **172 nodes across 23 screens**. One cause: the status palette used as text, where `DEFAULT` runs 2.04:1–3.70:1 against a 4.5:1 minimum. The fix is the `-ink` tokens — mechanical, but ~189 class occurrences. `Badge.tsx` alone accounted for 105 of the original 277 and landed separately. Capped by a budget in `app-surface.spec.ts` so it can only go down.                                                                                                                                                                                      | P2       |
