...
| 21  | Desktop layout on supported sizes            | PARTIAL    | Playwright renders 42 screens in CI; that is a render, not a layout review                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| 22  | Accessibility checked with recorded evidence   | PARTIAL    | PARTIAL  | 80 Playwright assertions pass including axe scans of 14 public pages and 26 authenticated screens, at **0 contrast violations in both light and dark**. Improved in this pass: the update toast was the one global banner with no `role`, so it entered the DOM silently; it is now `role="status" aria-live="polite"` with a labelled dismiss. Still partial for one reason, not two: **no manual screen-reader pass has been done**. This row also cited GAP-030's status-palette debt as open; that row is struck closed in `docs/SAAS.md`, and citing a closed gap as a blocker is how a gate stays amber after the thing holding it amber was fixed |
...
2. ✅ **The delivery-audit migrations are merged and applied.** This step used to
   ask for `0117`, `0118` and `0119` to be reviewed as "written and verified but
   not merged", including the P0 where any manager could make themselves an owner
   by writing an invite row directly. They merged. Read live rather than inferred
   on 7 September 2026: `list_migrations` against the project returns every
   version through `0134`, so `0116` to `0119` are all in production. Nothing is
   waiting here.
...
## Run of 10 September 2026 — verification and repair pass

Not a full re-run of the gate: only the lines this pass actually exercised are
restated, and **every other line still stands at its 4 September value.** The
pass drove the built application against a local Supabase stack seeded with two
tenants, sixteen staff, a published week, an open amendment, leave, a swap and
clock events, signed in as an owner, a manager, two staff members, an owner with
no staff record and a platform owner.

| #   | Gate                                          | 4 Sep    | Now      | Evidence, 10 September 2026                                                                                                                                                                                                                                                                                                                                 |
| --- | --------------------------------------------- | -------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Application works online                      | PASS     | **PASS** | 49 routes opened per role across four roles after the repairs: no page error, no failed request, no error boundary. It was **not** true when the pass started — every staff member met an error boundary on `/app/dashboard` (GAP-118), which no gate here could see because none of them signs in as staff                                                 |
| 7   | Failed requests recover appropriately         | PARTIAL  | PARTIAL  | unchanged in substance; `0152` adds one genuinely detected conflict — a leave request queued offline over days already booked returns `23P01`, classifies permanent, and dead-letters with a sentence rather than replaying (`docs/OFFLINE-SPEC.md`)                                                                                                        |
| 8   | Duplicate submissions prevented or idempotent | PASS     | **PASS** | unchanged for replay, and now covers a second kind of duplicate: the same days cannot be booked off twice by one person, enforced by an exclusion constraint rather than by the form (`leave_no_double_booking.test.sql`, 10 assertions)                                                                                                                    |
| 18  | Database policies and permissions verified    | BLOCKED  | **PASS** | **run locally for the first time.** Docker is available on this machine, so `supabase db reset && supabase test db` applied all 152 migrations to an empty database and ran the full suite: **671 assertions across 72 files, all passing.** The note at gate 18 above — "Docker is not available on this machine" — was true when written and is no longer |
| 21  | Desktop layout on supported sizes             | PARTIAL  | PARTIAL  | unchanged. 18 management screens were also measured at 390px: `scrollWidth` equals `clientWidth` on every one, so nothing scrolls sideways on a phone. That is an overflow check, still not a layout review                                                                                                                                                 |
| 25  | Multi-tenant isolation                        | see SAAS | **PASS** | 29 tables probed over PostgREST with two organisations' real JWTs: no cross-tenant row, and staff self-promotion, self-approval of leave and payroll edits all refused at the database. A manager gets `42501` from every platform RPC; an anonymous visitor is sent to `/login` from every `/app/*` and `/admin/*` route                                   |

Two things this pass changed that a release reader should know:

- **Payroll figures were doubling while a rota amendment was open** (GAP-119).
  `/app/timesheets` read the week draft-inclusive and got both the published
  rota and the amendment that supersedes it — measured live at ten rows for five
  shifts and 74h planned against 37h rostered. Fixed in `listShiftsForPeriod`;
  any recorded hours figure taken from that screen before today should be
  re-read.
- **Beyond one API page is now demonstrated end to end.** 1,202 shifts on a
  single day were counted exactly by the attendance table, its export and its
  pagination, against `PGRST_DB_MAX_ROWS=1000`. Gate 20's "large data sets" line
  was previously untested.

Still blocked, unchanged and unaffected by this pass: real email and push
