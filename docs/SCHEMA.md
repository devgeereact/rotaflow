...
| `subscriptions`      | `org_id`, `plan` (`starter`\|`professional`\|`business`), `status`, `provider`, `provider_ref?`, `current_period_end?`                                                                                            | Billing seam, now wired to Stripe (`0050_stripe_billing.sql`, `supabase/functions/stripe-webhook`). `plans.stripe_price_id` maps each tier to its Stripe Price. |
