...
1. They are **run by hand**. No `npm` script invokes any of them and there is no
   `scripts/` directory.
...
| profile-security | `/app/account/security` | `docs/design/ProfileSecurity.png` | **Not matched.** `SecurityPage` ships password change and ending every other session, and `/app/account/sessions` lists sessions. MFA/TOTP, backup codes and trusted devices are still absent, and the ref's 100% "Security check-up" ring is deliberately not built |
