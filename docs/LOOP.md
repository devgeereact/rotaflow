...
2. They are **currently reachable in production** and should be `import.meta.env.DEV`-gated. See `docs/audit01.md` P1-1. When that lands, the loop is unaffected: it drives
...
| settings-billing    | `/app/settings/billing`               | `design/Settingsbilling.png`         | **Built, not ref-matched.** Stripe Checkout + Billing Portal wired (`0050`, `SettingsBillingPage.tsx`, `billingCheckoutService.ts`) — not verified against a real completed charge. See `docs/PRD.md` §5/§7                 |
