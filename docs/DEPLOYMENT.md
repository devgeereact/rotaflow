...
> **RotaFlow target:** domain `rotaflow.app` (placeholder until the real one is set),
> deployed into its **own** docroot — e.g. `~/rotaflow.app/` (addon domain) or
> `public_html/rotaflow/` (subpath). Replace `<domain>`/`<docroot>` below with the real
> values. If you deploy through a personal wrapper/CI, that tooling still has to obey the
> safety rules below — they exist because breaking them has caused real outages and leaks.
>
