-- =====================================================================
-- 0137_the_last_platform_owner_survives_a_race.sql
--
-- ## The defect
--
...
comment on function public.revoke_platform_role(uuid) is
  'Revokes a platform grant. Platform owners only. Locks the live owner set before counting it, so two concurrent revocations cannot both pass the last-owner guard (0137).';
comment on function public.grant_platform_role(uuid, text) is
  'Grants or changes a platform role. Platform owners only. Same lock as revoke: demoting the last owner empties the set just as revoking them does (0137).';
