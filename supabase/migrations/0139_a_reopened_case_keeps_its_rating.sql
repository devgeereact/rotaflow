-- =====================================================================
-- 0139_a_reopened_case_keeps_its_rating.sql
--
...
comment on function public.support_case_rating_needs_resolution() is
  'Refuses a rating on a case with no resolution, at the moment the rating is set. Replaces support_cases_csat_after_resolution, which expressed the same intent as a whole-row invariant and so blocked reopening a rated case with a bare 23514 (0139).';
...
comment on policy org_smtp_settings_write on public.org_smtp_settings is
  'An organisation owner manages its own mail settings. Operational platform staff may READ them (0139) so the console can say whether a tenant has its own sender rather than asserting it has none; the with-check is unchanged, so no platform role can write them. The password is withheld by org_smtp_settings_safe at the column level either way.';
