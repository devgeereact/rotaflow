-- =====================================================================
-- 0136_the_first_owner_can_be_invited_again.sql — restoring the bootstrap
-- branch `0126` dropped
--
-- ## The regression
--
...
comment on function public.create_invite(uuid, text, text, uuid, uuid) is
  'Creates an invitation. Owners and managers of the organisation, plus the 0052 bootstrap: a platform owner or administrator inviting the FIRST owner into an organisation with no memberships. Restored by 0136 after 0126 dropped it.';
...
comment on policy invites_select on public.invites is
  'Owners and managers of the organisation, plus a platform administrator reading the first-owner invitation for an organisation nobody has joined yet (0136). send-invite reads under the caller''s JWT, so without this branch it answers 404 for an invitation it just created.';
