-- =====================================================================
-- 0141_revoking_a_platform_role_ends_its_sessions.sql
--
-- ## The defect
--
...
-- ## Rollback
--
-- Restore `has_support_access` from `0140` and `revoke_platform_role` from
-- `0137`. Sessions already revoked by this stay revoked, which is the safe
-- direction.
-- =====================================================================
  ...
  select exists (
    select 1
     ...
     where s.org_id = p_org
       ...
       -- ends a live session immediately rather than at its expiry (0140).
       ...
       -- ended it (0141). The grant is read directly rather than through
...
comment on function public.has_support_access(uuid, boolean) is
  'Whether the caller holds a live support session for this organisation at the scope asked for, the customer still consents (0140), and the caller is still platform staff (0141). All three re-read per query, so revoking any of them ends an open session at once.';
...
begin
  ...
  -- different rows, leaving none (0137).
...
comment on function public.revoke_platform_role(uuid) is
  'Revokes a platform grant and ends any support session it was holding, in one transaction. Platform owners only. Locks the live owner set before counting it (0137); closes open sessions and audits how many (0141).';
