-- =====================================================================
-- 0143_consent_is_the_customers_and_it_ends_a_session.sql
--
...
-- to add a policy later that forgets it — the mistake `0122` and `0138` are
-- both records of.
--
-- ## Rollback
--
  ...
  select exists (
    select 1
     ...
     where s.org_id = p_org
       ...
       -- session immediately instead of at its natural expiry (0143).
       and o.support_access_allowed
  );
$$;

comment on function public.has_support_access(uuid, boolean) is
  'Whether the caller holds a live support session for this organisation, at the scope asked for, AND the customer still consents. Re-read per query, so withdrawing consent closes open sessions at once (0143).';
...
comment on function public.set_org_support_access(uuid, boolean) is
  'The customer''s consent to platform support opening their data. Organisation owners only, tested against memberships directly so no support session or platform role can set it (0143). Audited to both sides.';
